...
dust.helpers.getDateToNow = function (chunk, context, bodies, params) {
  var rezdate = dust.helpers.tap(params.date, chunk, context);
  var moment = require('moment');
  rezdate = moment(rezdate).fromNow();
  return chunk.write(rezdate);
};

dust.helpers.getYear = function (chunk, context, bodies, params) {
  var rezdate = dust.helpers.tap(params.text, chunk, context);
  return chunk.write(rezdate.substr(0,4));
};
